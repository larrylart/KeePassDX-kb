...
object PreferencesUtil {
	...
	///////////
	// Larry: added output device prefs
	// Output Device (Bluetooth dongle) selection
	fun getOutputDeviceId(context: Context): String? {
		val prefs = PreferenceManager.getDefaultSharedPreferences(context)
		return prefs.getString(context.getString(R.string.output_device_id_key), null)
	}

	fun setOutputDeviceId(context: Context, address: String?) {
		PreferenceManager.getDefaultSharedPreferences(context).edit().apply {
			if (address == null) remove(context.getString(R.string.output_device_id_key))
			else putString(context.getString(R.string.output_device_id_key), address)
			apply()
		}
	}
	...
	fun getOutputDeviceName(context: Context): String? {
		val prefs = PreferenceManager.getDefaultSharedPreferences(context)
		return prefs.getString(context.getString(R.string.output_device_name_key), null)
	}

	fun setOutputDeviceName(context: Context, name: String?) {
		PreferenceManager.getDefaultSharedPreferences(context).edit().apply {
			if (name == null) remove(context.getString(R.string.output_device_name_key))
			else putString(context.getString(R.string.output_device_name_key), name)
			apply()
		}
	}
	...
	fun setUseExternalKeyboardDevice(context: Context, enabled: Boolean) {
		...
		val keyId = res.getIdentifier("use_external_kb_device_key", "string", context.packageName)
		val key = if (keyId != 0) context.getString(keyId) else "use_external_kb_device"
	...
	fun useExternalKeyboardDevice(context: Context): Boolean {
		...
		// Prefer resource-based keys if they exist, otherwise fall back to a plain literal.
		val res = context.resources
		val keyId = res.getIdentifier("use_external_kb_device_key", "string", context.packageName)
		val key = if (keyId != 0) context.getString(keyId) else "use_external_kb_device"

		val defId = res.getIdentifier("use_external_kb_device_default", "bool", context.packageName)
		val def = if (defId != 0) res.getBoolean(defId) else false
		// for testing
		//val def = if (defId != 0) res.getBoolean(defId) else true
	...
	fun sendNewLineAfterPassword(context: Context): Boolean {
		val prefs = PreferenceManager.getDefaultSharedPreferences(context)
		val key = context.getString(R.string.output_device_send_new_line_key)
